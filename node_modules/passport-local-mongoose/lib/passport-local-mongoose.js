...
module.exports = function(schema, options) {
    ...
    // hungarian version
    options.incorrectPasswordError = options.incorrectPasswordError || 'Hibás jelszó';
    options.incorrectUsernameError = options.incorrectUsernameError || 'Hibás %s';
    options.missingUsernameError = options.missingUsernameError || 'A %s nincs kitöltve';
    options.missingPasswordError = options.missingPasswordError || 'Nincs megadva jelszó!';
    options.userExistsError = options.userExistsError || 'Ez az email cím már létezik: %s'; // Becuse the email is the usernem, but not with usernamefield option
    options.noSaltValueStoredError = options.noSaltValueStoredError || 'A hitelesítés nem lehetséges. A mongodb gyűjteményben nincs ilyen salt érték!';
    options.attemptTooSoonError = options.attemptTooSoonError || 'A bejelentkezési kísérlet túl hamar következett a megelező után';
    options.tooManyAttemptsError = options.tooManyAttemptsError || 'A fiók lezárza a túl gyakori kísérletek miatt';
